...
class WildResultProcessor
{
    ...
    public function findBestWildSubstitute(array $symbols): string
    {
        ...
        $symbolCounts = array_count_values(array_filter($symbols, fn($s) => $s !== self::SYMBOL_WILD));
    ...
    public function processWildSubstitutions(array $symbols, string $targetSymbol): array
    {
        return array_map(function($symbol) use ($targetSymbol) {
            return $symbol === self::SYMBOL_WILD ? $targetSymbol : $symbol;
        }, $symbols);
    }
    ...
    public function calculateWildContribution(
        ...
        $wildCount = count(array_filter($paylineSymbols, fn($s) => $s === self::SYMBOL_WILD));
        ...
        return [
            ...
            'wildCount' => $wildCount,
            'positions' => array_keys(array_filter($paylineSymbols, fn($s) => $s === self::SYMBOL_WILD))
        ];
    }
}
